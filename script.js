...
function loadProducts(category, sortBy = "NameFirst", sortOrder = "asc") {
    try {
        ...
        if (screens?.show) { const shown = screens.show('catalog', { action: 'loadProducts', data: [category, sortBy, sortOrder] }); if (shown) return shown; }
    } catch {}
...
window.__ssGetCatalogFlat = window.__ssGetCatalogFlat || function(){ try { if (Array.isArray(window.productsFlatFromServer) && window.productsFlatFromServer.length) return window.productsFlatFromServer; const base = (typeof window.products !== 'undefined' && window.products) ? window.products : ((typeof window.productsDatabase !== 'undefined' && window.productsDatabase) ? window.productsDatabase : (window.products || {})); return Object.values(base || {}).flat().filter(Boolean); } catch { return []; } };
