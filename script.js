...
function __ssGetBasketButtonEl() {
    const candidates = Array.from(document.querySelectorAll("#BasketButton, .BasketButton, .mobileBasketButton"));
...
function __ssShowBasketToastAddToCart({ qty, productName, optMsg }) {
    ...
    toast.appendChild(l1);
    toast.appendChild(l2);
    ...
    if (origin) {
        ...
        setTimeout(() => origin.classList.remove("ss-basket-pulse"), 280);
    }
...
function __ssNotifyAddToCart({ qty, productName, optMsg }) {
    const q = Math.max(1, parseInt(qty, 10) || 1);
    const name = String(productName || "").trim();

    if (USE_ADD_TO_CART_POPUP_BASKET_TOAST) {
        try {
            __ssShowBasketToastAddToCart({ qty: q, productName: name, optMsg: optMsg || "" });
            return;
        } catch { }
    }
...
function addToCart_legacy(productName, price, imageUrl, expectedPurchasePrice, productLink, productDescription, selectedOption = '') {// analytics: add to cart
    ...
    if (quantity > 0) {
        ...
        localStorage.setItem("basket", JSON.stringify(basket));
        __ssNotifyAddToCart({ qty: quantity, productName, optMsg: selectedOption ? ' (' + selectedOption + ')' : '' });
    } else {
...
function addToCart(productName, price, imageUrl, expectedPurchasePrice, productLink, productDescription, selectedOption = "", selectedOptions = null) {
    ...
    if (qty > 0) {
        ...
        __ssNotifyAddToCart({ qty, productName, optMsg });
    } else {
        alert("Please select at least one item.");
    }
}
