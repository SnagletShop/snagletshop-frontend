...
function __clearLegacyPaymentSuccessLocalStorage() {
    try {
...
function setPaymentSuccessFlag({ reloadOnOk = true } = {}) {
    try {
        ...
        if (!store) return;
        store.setItem(PAYMENT_SUCCESS_FLAG_KEY, "1");
        if (reloadOnOk) store.setItem(PAYMENT_SUCCESS_RELOAD_KEY, "1");
        else store.removeItem(PAYMENT_SUCCESS_RELOAD_KEY);
        // Remove any older persistent flags so the popup cannot respawn after the tab/site is closed.
...
function checkAndShowPaymentSuccess() {
    try {
        ...
        const flag = store.getItem(PAYMENT_SUCCESS_FLAG_KEY);
