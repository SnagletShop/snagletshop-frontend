    ...
    renderCatalogProducts(ctx = {}, category, sortBy = 'NameFirst', sortOrder = 'asc') {
      ...
      productList.forEach(product => {
        ...
        const productDiv = ctx.createProductCard?.(product, {
          ...
          onOpenProduct: () => ctx.navigate?.('GoToProductPage', [product.name, (ctx.resolveVariantPriceEUR?.(product, [], '') || product.price), ((ctx.getABProductDescription?.(product) || product.description) || ctx.TEXTS?.PRODUCT_SECTION?.DESCRIPTION_PLACEHOLDER)]),
          onDecrease: (key) => ctx.decreaseQuantity?.(key),
          onIncrease: (key) => ctx.increaseQuantity?.(key),
          onAddToCart: () => ctx.addToCart?.(product.name, product.price, product.image, product.expectedPurchasePrice, product.productLink, (ctx.getABProductDescription?.(product) || product.description), '', ctx.defaultSelectedOptions?.(ctx.extractOptionGroups?.(product)), (product.productId || null))
